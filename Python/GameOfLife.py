...
class Cell (object):
	...
	def update(self, b=None):
		if b is not None:
			self.alive = b
		self.background = Color(0,1,0) if self.alive else Color(0,0,0)
...
class Grid (object):
	def __init__(self, w, h):
		...
		for x in xrange(self.size.w): grid_draw.line((x*w, 0, x*w, screen.h,), 'Gray') # vertical lines
		for y in xrange(self.size.h): grid_draw.line((0, y*h, screen.w, y*h), 'Gray') # orizontal lines
	...
	def update(self, by_random=False):
		kill = []
		life = []		
		# RULES
		# for dead_cells
		for cell in self.dead_cells():
			...
			n = random.randint(0,10) if by_random == True else 3
			if len(cell.living_neighbors()) == n:
				life.append(cell)
				
		# for living_cells		
		for cell in self.living_cells():
			# value_when_true if condition else value_when_false
			n = random.randint(0,10) if by_random == True else 1
			m = random.randint(0,10) if by_random == True else 3
			if len(cell.living_neighbors()) < n:
				kill.append(cell)
			elif len(cell.living_neighbors()) > m:
				kill.append(cell)
			else:
				life.append(cell)
		# update		
	...
	def draw(self):
		stroke(0,0,0) # living cell stroke color
		stroke_weight(0) # living cell stroke size
